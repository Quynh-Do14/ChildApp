...
class NotificationService {
     ...
     */
    async registerDeviceToken(token: string, deviceName?: string) {
        try {
            console.log('Registering device token with backend:', { token, deviceName });
            ...
            return await RequestService.post(
                `${Endpoint.Notification?.RegisterToken || '/device-tokens/register'}`,
                {
                    ...
                    deviceName: deviceName || `${Platform.OS.toUpperCase()} Device`
                }
            ).then(response => {
